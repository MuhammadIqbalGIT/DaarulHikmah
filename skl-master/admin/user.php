...
?>
	<div class="container">
		...
		while ($data = mysqli_fetch_array($qUser)) {
			...
			if ($data['UID'] == '1' || $data['UID'] == '5' || $data['UID'] == '6') {
                echo '<td>';
                echo '<button class="btn btn-primary btn-sm" disabled="disabled"><span class="glyphicon glyphicon-pencil" aria-hidden="true"></span> Edit</button> ';
                echo '<button class="btn btn-danger btn-sm" disabled="disabled"><span class="glyphicon glyphicon-trash" aria-hidden="true"></span> Delete</button>';
                echo '</td>';
            } else {
                echo '<td>';
                echo '<a href="user_edit.php?uid=' . $data['UID'] . '" class="btn btn-primary btn-sm"><span class="glyphicon glyphicon-pencil" aria-hidden="true"></span> Edit</a> ';
                echo '<a href="user_del.php?uid=' . $data['UID'] . '" class="btn btn-danger btn-sm" onclick="return confirm(\'Yakin akan menghapus user ini?\')"><span class="glyphicon glyphicon-trash" aria-hidden="true"></span> Delete</a>';
                echo '</td>';
            }
