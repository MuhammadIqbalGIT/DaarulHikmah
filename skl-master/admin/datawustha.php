...
<body>
    ...
    <div class="container">
        <h2><b>Data Kelulusan Tingkat Wustha</b></h2>
    </div>
    <div class="row">
        <div class="container">
            <!-- input Cari data -->
            <form action="datawustha.php" method="get">
            ...
            <div class="table table-responsive wrapper">
                <table class="table table-bordered table-hover table-striped table-fixed header-fixed table-fixed head">
                    <thead style="position: sticky; top: 0; z-index: 100; background: white;">
                        <tr class="">
                            ...
                            <th colspan="13" class="bg-primary text-center">Nilai Akhir Mata Pelajaran</th>
                    ...
                    <tbody>
                        <?php
                            ...
                            if (isset($_GET['cari'])) {
                                ...
                                $qsiswa = mysqli_query($db_conn, "SELECT * FROM data_wustha WHERE no_ujian like '%" . $cari . "%' OR nama like '%" . $cari . "%' OR kls like '%" . $cari . "%' ORDER BY no_ujian ASC");
                            } else if (isset($_GET['by'])) {
                                $sort = $_GET['by'];
                                switch ($sort) {
                                    case 'no_ujiana':
                                        $qsiswa = mysqli_query($db_conn, "SELECT * FROM data_wustha ORDER BY no_ujian ASC");
                                        break;
                                    case 'no_ujianb':
                                        $qsiswa = mysqli_query($db_conn, "SELECT * FROM data_wustha ORDER BY no_ujian DESC");
                                        break;
                                    case 'namaa':
                                        $qsiswa = mysqli_query($db_conn, "SELECT * FROM data_wustha ORDER BY nama ASC");
                                        break;
                                    case 'namab':
                                        $qsiswa = mysqli_query($db_conn, "SELECT * FROM data_wustha ORDER BY nama DESC");
                                        break;
                                    case 'klsa':
                                        $qsiswa = mysqli_query($db_conn, "SELECT * FROM data_wustha ORDER BY kls ASC");
                                        break;
                                    case 'klsb':
                                        $qsiswa = mysqli_query($db_conn, "SELECT * FROM data_wustha ORDER BY kls DESC");
                                        break;
                                    case 'statusa':
                                        $qsiswa = mysqli_query($db_conn, "SELECT * FROM data_wustha ORDER BY status ASC");
                                        break;
                                    case 'statusb':
                                        $qsiswa = mysqli_query($db_conn, "SELECT * FROM data_wustha ORDER BY status DESC");
                                        break;
                            ...
                            if (mysqli_num_rows($qsiswa) > 0) {
                                while ($data = mysqli_fetch_array($qsiswa)) {
                                    ...
                                    echo '<td class="text-center">
										<a href="#" 
										   data-toggle="modal" 
										   data-target="#exampleModalLongWustha"
                ...
                <form action="../print/print_wustha.php" method="POST" target="_blank">
        ...
        <!-- Modal Detail Siswa-->
        <div class="modal fade" id="exampleModalLongWustha" tabindex="-1" role="dialog"
            ...
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        ...
                        <form id="formInputData" method="POST" action="edit_data_wustha.php">
        ...
        <script>
        $('#exampleModalLongWustha').on('show.bs.modal', function(event) {
        ...
        $('#exampleModalLongWustha').on('show.bs.modal', function(event) {
            ...
            modal.find('.modal-title').html(
                'Detail Data Siswa Siswi Tingkat Wustha <strong>(<span style="font-size: 1.2em;">' +
        ...
        <div class="modal fade" id="exampleModalLongAddSiswa" tabindex="-1" role="dialog"
            ...
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="exampleModalLongTitle">Tambah Data Siswa Siswi Tingkat Wustha</h5>
                        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                            <span aria-hidden="true">&times;</span>
                        </button>
                    </div>
                    <div class="modal-body">
                        <!-- Form untuk menambah data siswa -->
                        <form id="formInputData" method="POST" action="add_data_wustha.php">
                            <div class="form-group">
                                <label for="no_ujian">No Ujian</label>
                                <input type="text" class="form-control" id="no_ujian" name="no_ujian"
                                    placeholder="Masukkan No Ujian">
