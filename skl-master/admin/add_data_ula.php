...
if (isset($_SESSION['logged']) && !empty($_SESSION['logged'])) {
    ...
    $stmt_check = $db_conn->prepare("SELECT COUNT(*) FROM `data_ula` WHERE `no_ujian` = ?");
    ...
    if (empty($message)) {
        $stmt_check = $db_conn->prepare("SELECT COUNT(*) FROM `data_ula` WHERE `nis` = ?");
    ...
    if (empty($message)) {
        $stmt_check = $db_conn->prepare("SELECT COUNT(*) FROM `data_ula` WHERE `nisn` = ?");
    ...
    if (!empty($message)) {
        ...
        $query = "INSERT INTO `data_ula` (`no_ujian`, `nis`, `nisn`, `nama`, `ttl`, `ortu`, `kls`, `n_alquran`, `n_hadits`, 
    `n_aqidah`, `n_akhlaq`, `n_fiqih`, `n_tarikh_sejarah_peradaban_islam`, `n_bahasa_arab`, `n_ppkn`, 
    `n_bahasa_indonesia`, `n_matematika`, `n_ipa`, `n_ips`, `rata2`, `status`) 
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";

        // Perbaiki jumlah karakter tipe data agar sesuai dengan parameter
        $stmt = $db_conn->prepare($query);
        $stmt->bind_param(
            "sssssssssssssssssddss", // Sesuaikan dengan 20 parameter: "s" untuk string, "d" untuk decimal
            $no_ujian,
            $nis,
            $nisn,
            $nama,
